...
class GeoServer(object):
    ...
    def getFormat(self):
        return self._getProperty(
            'format',
            'application%2Fvnd.google-earth.kml%2Bxml'
        )
    ...
    def getLayers(self):
        ...
        return [i.find('name').text for i in parser.findAll(
            'layer', queryable="1")
        ]
    ...
    def getSRid(self):
        ...
        content = self._makeCall(url)
        parser = BeautifulSoup(content)
        return [i.text for i in
                parser.wms_capabilities.capability.layer.findAll('crs')]
